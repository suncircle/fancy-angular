 ...
 */
return angular.module('pascalprecht.translate').provider('$fancyAngularLocalesLoader', function () {
  function Part(name) {
    this.name = name;
    this.isActive = true;
    this.tables = {};
  ...
  Part.prototype.getTable = function (lang, $q, errorHandler) {
    var deferred = $q.defer();
    if (!this.tables[lang]) {
      var self = this;
      
        var proceed = function (locale) {
            self.tables[lang] = locale;
            deferred.resolve(locale);
        };
        var locale = 'fancyPlugin!locale:' + self.name + ':' + lang;
        //deferred.promise.required = locale;
        require([locale],
            function requireSuccess(locale){
                if (typeof locale == typeof 'string') {
                    require(['json'], function(json){
                        proceed(json.parse(locale))
                    });
                }else{
                    proceed(locale)
                }
      ...
      var service = function (options) {
        ...
        for (var part in parts) {
          if (hasPart(part) && parts[part].isActive) {
            loaders.push(parts[part].getTable(options.key, $q, errorHandler).then(addTablePart));
          }
        }
        if (loaders.length) {
